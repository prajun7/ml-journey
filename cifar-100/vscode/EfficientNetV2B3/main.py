...
from tensorflow.keras.applications.efficientnet_v2 import EfficientNetV2B3  # Added for EfficientNetV2B3
...
print(f"  Min/Max pixel values: {x_train.min()}/{x_test.max()}") # 0/255
...
# Defining the Keras Sequential model using EfficientNetV2B3 (with Batch Norm, L2, Softmax output)...
print("Defining the Keras Sequential model using EfficientNetV2B3 (with Batch Norm, L2, Softmax output)...")

# Load EfficientNetV2B3 with pre-trained ImageNet weights, excluding top layers
...
print("Model defined successfully with EfficientNetV2B3, Batch Norm, L2, and Softmax output.")
model.summary()

# Compile the model with label smoothing
model.compile(
    optimizer=optimizers.Adam(learning_rate=LEARNING_RATE),
    loss=losses.SparseCategoricalCrossentropy(label_smoothing=0.1),
    metrics=['accuracy']
)

# Train the model (Phase 1: Train top layers)
print("\nTraining Phase 1: Training top layers with base model frozen...")
history = model.fit(
    train_dataset,
    epochs=NUM_EPOCHS,
...
# Fine-tuning (Phase 2: Unfreeze last 20 layers)
print("\nTraining Phase 2: Fine-tuning last 20 layers of EfficientNetV2B3...")
...
# Plotting training history (combine both phases)
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.title('Model Accuracy over Epochs (Initial + Fine-Tuning)') 
plt.ylim([0, 1])
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Model Loss over Epochs (Initial + Fine-Tuning)') 
